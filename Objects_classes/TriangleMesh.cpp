...
TriangleMesh::TriangleMesh(const std::shared_ptr<Entity> &e) {
    ...
    for(int i = 0; i < e->vertices.size(); i+= 3){
        ...
        auto new_triangle = std::make_shared<Triangle>(coords[0], coords[1], coords[2], UVpoints[0], UVpoints[1], UVpoints[2], normals[0], normals[1], normals[2], e->texture, e->material);
