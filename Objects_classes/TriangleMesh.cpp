...
TriangleMesh::TriangleMesh(const std::shared_ptr<Entity> &e) {
    ...
    for(int i = 0; i < e->vertices.size(); i+= 3){
        ...
        for(auto j = 0; j < 3; j ++){
            ...
            normals[j] =  glm::vec3(e->worldMatrix * glm::vec4(vertex.Norm, 0));
            //color_mean += vertex.Color;
        }

        normal_mean *= 1.0/3;

        auto new_triangle = std::make_shared<Triangle>(coords[0], coords[1], coords[2], normal_mean, UVpoints[0], UVpoints[1], UVpoints[2], normals[0], normals[1], normals[2], e->texture, e->material);
